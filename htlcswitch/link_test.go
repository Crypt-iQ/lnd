...
func customShutdownScriptInternal(t *testing.T, setShutdown bool,
	...
	coreLink.cfg.NotifyShutdownSent = func(req *ChanClose,
		quit chan struct{}) {
	...
	// Ensure we receive the NotifyShutdownSent event.
	select {
...
func TestDuplicateShutdown(t *testing.T) {
	...
	case <-closeReqChan:
	case <-time.After(5 * time.Second):
		t.Fatalf("expected to receive NotifyShutdownSent event")
	}
...
func TestShutdownInit(t *testing.T) {
	...
	aliceLink.NotifyShouldShutdown(closeReq)
