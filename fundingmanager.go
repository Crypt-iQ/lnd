...
func (f *fundingManager) Start() error {
	...
	for _, channel := range pendingChannels {
		...
		go func(ch *channeldb.OpenChannel) {
			...
			go f.waitForFundingWithTimeout(ch, signalChan, timeoutChan)

			// goroutine for channel announcements
			go f.announceChannelAfterFundingLocked(announceDoneChan, ch, signalChan)
			...
			case <-announceDoneChan:
			// Success, channel was announced.
			}
		}(channel)
	}
	...
	for _, channel := range openChannels {
		...
		case fundingLockedSent:
			...
			go func() {
				...
				f.sendChannelAnnouncement(channel,
...
func (f *fundingManager) handleFundingSigned(fmsg *fundingSignedMsg) {
	...
	go func() {
		...
		go func() {
			defer f.wg.Done()
			// TODO - 305
			f.waitForFundingConfirmation(completeChan,
				doneChan, cancelChan, signalChan)
		}()
...
// process once the funding transaction has been broadcast. The primary functions
// of waitForFundingConfirmation are to wait for blockchain confirmation, send
// the fundingLocked message, add the channel to our internal channel graph, and
// to notify a separate goroutine to start the channel announcement if the correct
// number of confirmations have been reached.  The wait can be canceled by
// closing the cancelChan.
func (f *fundingManager) waitForFundingConfirmation(completeChan *channeldb.OpenChannel,
	...
	signalChan chan<- *signalAnnounceChannel) {
	...
	f.sendFundingLocked(completeChan, &shortChanID, signalChan)
}
...
func (f *fundingManager) sendFundingLocked(
	...
	// Note: we don't call `defer channel.Stop()` since we want it running
	// when announceChannelAfterFundingLocked is called.
	...
	// if and only if it has reached the correct number of confirmations.
...
func (f *fundingManager) announceChannelAfterFundingLocked(doneChan chan<- struct{},
	...
	// Receive signal
	signalAnn, ok := <-signalChan
	if !ok {
		// TODO - 305
		return
	}

	channel := signalAnn.channel
	shortChanID := signalAnn.shortChanID
