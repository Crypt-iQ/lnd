...
func (c *ChanCloser) ChannelClean() ([]lnwire.Message, bool, error) {
	...
	// Calculate the idealFeeSat in relation to the commit fee here since
	// now the channel state cannot be updated. If this fee is greater than
	// the fee currently present within the commitment transaction, then
	// we'll clamp it down to be within the proper range.
	//
	// TODO(roasbeef): clamp fee func?
	channelCommitFee := c.Channel().StateSnapshot().CommitFee
	...
	chancloserLog.Infof("Ideal fee for closure of ChannelPoint(%v) is: %v"+
		" sat", c.Channel().ChannelPoint(), int64(c.idealFeeSat))
