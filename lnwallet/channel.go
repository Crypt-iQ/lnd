...
// ignored.
func (u *updateLog) restoreHtlc(pd *PaymentDescriptor) bool {
...
func (u *updateLog) restoreUpdate(pd *PaymentDescriptor) bool {
	if _, ok := u.updateIndex[pd.LogIndex]; ok {
		return false
	}
...
func (lc *LightningChannel) restorePendingLocalUpdates(
	...
	for _, logUpdate := range pendingRemoteCommitDiff.LogUpdates {
		...
		case Add:
			...
			if lc.localUpdateLog.restoreHtlc(payDesc) {
				lc.localUpdateLog.htlcCounter++
				lc.localUpdateLog.logIndex++
			}
		case FeeUpdate:
			if lc.localUpdateLog.restoreUpdate(payDesc) {
				lc.localUpdateLog.logIndex++
			}
		default:
			if lc.localUpdateLog.restoreUpdate(payDesc) {
